...
def run(dag_id, task_id, execution_date, env, poll_interval):
    ...
    clear_url = "http://{host}:{port}/admin/rest_api/api?api=clear&dag_id={dag_id}&task_regex=^{task_id}$&" \
                "start_date={execution_date}&end_date={execution_date}".format(host=host, port=port, dag_id=dag_id,
                                                                               task_id=task_id,
